...
def _load_review_log() -> dict:
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "content", "review-log.json")
    try:
        with open(path, encoding="utf-8") as fh:
            return json.load(fh).get("reviewed", {})
...
def reviewed_iso(slug: str) -> str:
    """Date this page's cited facts were last re-checked, or '' if never.

    A page counts as reviewed when someone actually opened its sources and
    confirmed them — not when the site happened to be rebuilt. Stamping build
    day on all 23 pages would claim 23 fact-checks for one, so an unreviewed
    page shows no date rather than an unearned one.
    """
    return REVIEW_LOG.get(slug, "")


def reviewed_human(slug: str) -> str:
    stamp = reviewed_iso(slug)
    return date.fromisoformat(stamp).strftime("%B %-d, %Y") if stamp else ""
...
def build_footer(slug: str = "") -> str:
    reviewed = reviewed_human(slug)
    reviewed_line = (
        f"<p>Page last reviewed {reviewed}. Facts on this page were re-checked "
        f"against their cited sources on that date.</p>" if reviewed else ""
    )
    cols = ""
...
def build_page(slug: str) -> str:
    ...
    reviewed = reviewed_human(slug)
    reviewed_suffix = f" &middot; Last reviewed {reviewed}" if reviewed else ""
...
<article>
  <h1>{meta['h1']}</h1>
  <p class="updated">Written for Calgary homeowners{reviewed_suffix}</p>
...
def main():
    # A page is a .json with a matching .html body. review-log.json (and any
    # other bookkeeping JSON) has no body, so it is not a page.
    slugs = sorted(
        f[:-5] for f in os.listdir(CONTENT)
        if f.endswith(".json")
        and os.path.exists(os.path.join(CONTENT, f[:-5] + ".html"))
    )
    ...
    entries = "\n".join(
        ...
        for u, p, m in sorted(urls, key=lambda x: (-float(x[1]), x[0]))
    )
